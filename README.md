...
1. static  (**19장 내용 정정**)
    - 정적 키워드는 소스코드가 동작하는 동안 끝까지 살아있는 변수나 함수를 뜻함.
    - static으로 선언하면 자기 소스내에서는 위와같이 동작
    - 외부 소스로 공유하지 않음 <-> `객체지향 언어와는 정반대`.
    - 외부 소스에 공개하지 않을 변수를 static 으로 지정
    - 두 소스에 같은 이름의 함수나 변수를 사용하는데 중복문제를 해결하기 위함.

2. 토이 프로젝트 - [주소록 프로그램](./TOYPROJECT.md)